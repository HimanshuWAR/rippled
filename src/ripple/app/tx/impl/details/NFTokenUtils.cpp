    ...
    std::function<void(ApplyView&, AccountID const&)> const& createCallback)
{
    ...
    auto narr = cp->getFieldArray(sfNonFungibleTokens);
...
{
    static uint256 constexpr nftPageMask(std::string_view(
        "0000000000000000000000000000000000000000ffffffffffffffffffffffff"));

    return (a & nftPageMask) < (b & nftPageMask);
}
...
removeToken(ApplyView& view, AccountID const& owner, uint256 const& tokenID)
{
    ...
    // of the page we removed (if any) into one.
    if (prev && next &&
        mergePages(
            view,
