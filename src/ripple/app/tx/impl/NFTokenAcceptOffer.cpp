...
NFTokenAcceptOffer::preflight(PreflightContext const& ctx)
{
    ...
    // present and greater than zero in bridged mode.
...
NFTokenAcceptOffer::preclaim(PreclaimContext const& ctx)
{
    ...
    {  // Bridged mode:
        ...
        // The two offers being bridged must be for the same token:
        if ((*bo)[sfTokenID] != (*so)[sfTokenID])
            return tecBUY_SELL_MISMATCH;

        // The two offers being bridged must be for the same asset:
        if ((*bo)[sfAmount].issue() != (*so)[sfAmount].issue())
            return tecBUY_SELL_MISMATCH;

        // Ensure that the buyer is willing to paying at least as much as the
        // seller is requesting:
        if ((*so)[sfAmount] > (*bo)[sfAmount])
            return tecINSUFFICIENT_PAYMENT;

        // The bridger can specify an amount that represents their cut; if they
    ...
    if (sell)
    {
        ...
        // If not in bridged mode, the seller must own the token:
        if (!buy && !nft::findToken(ctx.view, (*so)[sfOwner], (*so)[sfTokenID]))
            return tecNO_PERMISSION;
...
NFTokenAcceptOffer::doApply()
{
    ...
    if (bo && so)
    {
        ...
        // Send the bridger the amount they requested
        if (auto const cut = ctx_.tx[sfAmount];
            amount != beast::zero && cut != beast::zero)
        {
            if (auto const r = pay(buyer, account_, cut); !isTesSuccess(r))
                return r;
