...
NFTokenMint::doApply()
{
    std::uint32_t tsn = std::numeric_limits<std::uint32_t>::max();

    auto const issuer = ctx_.tx[~sfIssuer].value_or(account_);

    if (auto const root = view().peek(keylet::account(issuer)))
    {
        // Get the unique serial number for this token:
        tsn = (*root)[~sfMintedTokens].value_or(0);
        (*root)[sfMintedTokens] = tsn + 1;
        ctx_.view().update(root);
    }

    if (tsn == std::numeric_limits<std::uint32_t>::max())
    ...
    auto ret = nft::insertToken(
        ...
        {*InnerObjectFormats::getInstance().findSOTemplateBySField(
             sfNonFungibleToken),
         sfNonFungibleToken,
         [this, issuer, tsn](STObject& object) {
             object.setFieldH256(
                 sfTokenID,
                 createTokenID(
